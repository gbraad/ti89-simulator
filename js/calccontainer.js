...
(function( window, document ) {
    ...
    var wrapStoreData = function( obj, data ) {
        ...
        } else if (obj.type == 'send-compress') {
            var req_data = new FormData();
            req_data.append('data', obj.data);

            var xhr = new XMLHttpRequest();
            xhr.open('POST', 'https://ti89-simulator.com/compress.php', true);
            xhr.onload = function () {
...
$(function() {
  ...
  function mapKeyboardButton(btnid, key, imgid) {
    ...
    button_el.addEventListener("mousedown", function() { startUT(); emu.setKey(key, 1); image_el.style.display = "block"; });
    button_el.addEventListener("touchstart", function() { startUT(); emu.setKey(key, 1); image_el.style.display = "block"; });
    button_el.addEventListener("mouseup", function() { startUT(); emu.setKey(key, 0); image_el.style.display = "none"; });
    button_el.addEventListener("touchend", function() { startUT(); emu.setKey(key, 0); image_el.style.display = "none"; });
    button_el.addEventListener("touchleave", function() { startUT(); emu.setKey(key, 0); image_el.style.display = "none"; });
    button_el.addEventListener("touchcancel", function() {startUT();  emu.setKey(key, 0); image_el.style.display = "none"; });
  }
